...
export default function EventList() {
        ...
        try {
            ...
            setEvents(prevEvents =>
                prevEvents.map(event =>
                    event._id === eventId ? { ...event, approvalStatus: newStatus } : event
                )
            );
    ...
    if (events.length === 0) {
        return <div className="text-center p-8 text-gray-500 dark:text-gray-400">No events found in the queue.</div>;
    }
    ...
    const EventCard = ({ event }) => (
        <div className="bg-white dark:bg-gray-800 p-4 shadow-lg rounded-lg border border-gray-200 dark:border-gray-700 lg:hidden transition-shadow hover:shadow-xl">
            <div className="flex justify-between">
                <div>
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100">{event.eventName || 'Untitled Event'}</h3>
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Email: {event.organizerEmail.length > 8
                    ? event.organizerEmail.slice(0, 8) + "..."
                    : event.organizerEmail}</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">
                        Start Date: {event.startDateTime ? new Date(event.startDateTime).toLocaleDateString() : 'N/A'}
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">
                        End Date: {event.endDateTime ? new Date(event.endDateTime).toLocaleDateString() : 'N/A'}
                    </p>
                    Status: <span className={`inline-flex items-center px-3 py-1 text-xs font-medium rounded-full ${getStatusStyle(event.approvalStatus)}`}>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {events.map((event) => (
                            <tr key={event._id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-gray-100">{event.eventName || 'N/A'}</td>
                                <td className="px-6 py-4 cursor-pointer text-sm font-medium text-gray-900 dark:text-gray-100" title={`${event.organizerEmail}`}  >{event.organizerEmail.length > 8
                    ? event.organizerEmail.slice(0, 8) + "..."
                    : event.organizerEmail}</td>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className={`px-3 py-1 text-xs font-medium rounded-full ${getStatusStyle(event.approvalStatus)}`}>
                                        {event.approvalStatus}
                                    </span>
                                </td>
